...
def add_self_loops_ref(a, fill=1.0):
    """
    ...
    indices = tf.concat((indices_od, indices_sl), 0) # error
...
def add_self_loops(a, fill=1.0):
    ...
    those node that don't have a self-loop already, and are assigned a weight
    of `fill`.
    :param a: a square SparseTensor. # batch
    :param fill: the fill value for the new self-loops. It will be cast to the
    dtype of `a`.
    :return: a SparseTensor with the same shape as the input.
    """
    ...
    m = tf.shape(a, out_type=indices.dtype)[0]
    N = tf.shape(a, out_type=indices.dtype)[-1]

    mask_od = indices[:, -2] != indices[:, -1]
    mask_sl = ~mask_od
    mask_od.set_shape([None])  # For compatibility with TF 2.2
    mask_sl.set_shape([None])
    ...
    values_sl = tf.fill((m*N,), tf.cast(fill, values.dtype))
    values_sl = tf.tensor_scatter_nd_update(
        values_sl, indices_sl[:, 0:1]*m+indices_sl[:, 1:2], values[mask_sl]
    )

    indices_sl = tf.range(N, dtype=indices.dtype)[:, None]
    indices_sl = tf.repeat(indices_sl, 2, -1)
    indices_sl = tf.tile(indices_sl, [m, 1])
    head = tf.range(m, dtype=indices.dtype)[:, None]
    head = tf.repeat(head, N, 0)
    indices_sl = tf.concat((head, indices_sl), axis=-1)

    indices_sl = tf.cast(indices_sl, indices.dtype)
    ...
    out = tf.SparseTensor(indices, values, (m, N, N))

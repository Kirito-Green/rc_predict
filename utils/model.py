...
from config import *
from utils.analysis import error_analysis, scatter_plot
...
def gnn_plot(model, batch_size, x_train, y_train, x_valid, y_valid, x_test, y_test, dir, name):
	...
	x_valid_loader = BatchLoader(x_valid, batch_size=batch_size, shuffle=False)
	x_test_loader = BatchLoader(x_test, batch_size=batch_size, shuffle=False)
	y_train_pred = model.predict(x_train_loader.load(), steps=x_train_loader.steps_per_epoch).reshape(-1, 1)
	y_valid_pred = model.predict(x_valid_loader.load(), steps=x_valid_loader.steps_per_epoch).reshape(-1, 1)
	y_test_pred = model.predict(x_test_loader.load(), steps=x_test_loader.steps_per_epoch).reshape(-1, 1)

	# 3 pictures
	scatter_plot(y_train, y_train_pred, dir, f'{name}_train')
	scatter_plot(y_valid, y_valid_pred, dir, f'{name}_valid')
	scatter_plot(y_test, y_test_pred, dir, f'{name}_test')
